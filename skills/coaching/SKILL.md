...
**Schedule validation is part of setup.** For a daily 06:10 job use a 5-field cron expression such as `10 6 * * *`; for Sunday 17:00 use `0 17 * * 0`. After creating or updating a job, inspect the returned `schedule` and `next_run_at` and confirm the weekday/time in the athlete's configured timezone. Do not assume a technically accepted expression means the intended time—an extra cron field can change the meaning silently. Keep morning and weekly-plan jobs separate, with explicit names and `deliver: origin` only when the origin is the intended athlete.
