...
```bash
cd /opt/data && HERMES_HOME=/opt/data python3 -c "
import sys, os, json
sys.path.insert(0, 'plugins/training')
from intervals_icu import (
    get_athlete_profile, get_wellness, get_planned_events,
    get_sport_settings, get_recent_activities, get_power_curve,
    get_fitness_chart
)
uid = 'SNOWFLAKE_HERE'
...
show('PROFILE', get_athlete_profile)
show('WELLNESS', get_wellness, 3)
show('EVENTS', get_planned_events, 1)
show('SPORT_SETTINGS', get_sport_settings, 'Ride')
show('RECENT_ACTIVITIES', get_recent_activities, 14)
show('POWER_CURVE', get_power_curve, 'Ride', 42)
show('FITNESS', get_fitness_chart, 365)
" 2>&1
```
